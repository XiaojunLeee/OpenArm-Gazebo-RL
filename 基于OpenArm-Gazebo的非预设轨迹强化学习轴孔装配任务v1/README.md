...
其中
$a_Y$，
$a_Z$的范围都是[-1,1]，表示下一部动作朝Y、Z的正/负方向移动的权重。
$a_Y$， 
$a_Z$ 只是表示下一步的动作，而并非真实移动的距离，目前v1方案使用的是**位置增量**的方法，就是设定一个最大的动作步长
$\Delta{a_{yz}}=0.2mm$
，即
...
$Z$为轴心，
$Y_{hole}$、
$Z_{hole}$为孔心。当满足上述条件以后，判定为找到孔心，此时SAC不再控制机器人，进入INSERTING阶段，交回底层控制器负责后续插入。
