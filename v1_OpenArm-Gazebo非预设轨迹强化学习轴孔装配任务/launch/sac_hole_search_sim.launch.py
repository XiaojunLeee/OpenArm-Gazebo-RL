"""SAC 搜索专用启动入口；原 ``hole_search_sim.launch.py`` 保持不变。"""
...
def generate_launch_description():
    """启动同一 Gazebo 世界，只把原螺旋节点换为 RL 搜索节点。"""
    ...
    controller_config = PathJoinSubstitution(
        [package_share, 'config', 'sac_search.yaml'])
    ...
    spawn_robot = Node(
        package='ros_gz_sim', executable='create', output='screen', arguments=[
            '-name', 'openarm_right', '-topic', 'robot_description',
            '-allow_renaming', 'false'])
    clock_bridge = Node(
        package='ros_gz_bridge', executable='parameter_bridge', output='screen',
        arguments=[
            '/clock@rosgraph_msgs/msg/Clock[ignition.msgs.Clock',
            # 每个 Episode 删除并重建四块板，使外边界固定、真实深方孔
            # 按随机 Y/Z 更新；孔位同时写入控制器规则判定参数。
            '/world/hole_search/remove@ros_gz_interfaces/srv/DeleteEntity',
            '/world/hole_search/create@ros_gz_interfaces/srv/SpawnEntity',
        ])
    ...
    effort_spawner = Node(
        package='controller_manager', executable='spawner', output='screen',
        arguments=['arm_effort_controller', '--controller-manager',
                   '/controller_manager', '--controller-manager-timeout', '60',
                   '--inactive'])
    # 只有这里使用 RL 节点；其余启动顺序与原入口一致。
    controller = Node(
        package='openarm_gazebo_hole_search',
        executable='sac_runtime_controller', name='rl_hole_search_controller',
        output='screen', parameters=[controller_config])
    rviz = Node(
        package='rviz2', executable='rviz2', output='screen',
        arguments=['-d', rviz_config], parameters=[{'use_sim_time': True}],
        condition=IfCondition(LaunchConfiguration('rviz')))
    unpause_world = ExecuteProcess(
        cmd=['ign', 'service', '-s', '/world/hole_search/control',
             '--reqtype', 'ignition.msgs.WorldControl',
             '--reptype', 'ignition.msgs.Boolean', '--timeout', '3000',
             '--req', 'pause: false'], output='screen')
    activate_controllers = ExecuteProcess(
        cmd=['ros2', 'control', 'switch_controllers', '--activate',
             'joint_state_broadcaster', 'arm_effort_controller', '--strict',
             '--controller-manager', '/controller_manager'], output='screen')
    return LaunchDescription([
        DeclareLaunchArgument('rviz', default_value='true'),
        SetEnvironmentVariable('IGN_GAZEBO_RESOURCE_PATH', [
            description_resource_parent, ':', EnvironmentVariable(
                'IGN_GAZEBO_RESOURCE_PATH', default_value='')]),
        SetEnvironmentVariable('GZ_SIM_RESOURCE_PATH', [
            description_resource_parent, ':', EnvironmentVariable(
                'GZ_SIM_RESOURCE_PATH', default_value='')]),
        gazebo, clock_bridge, robot_state_publisher, spawn_robot,
