...
### 20260813v3——基于SAC算法仅引入于SEARCHING状态的强化学习轴孔装配任务
...
进入SEARCHING阶段后，先记录目前轴尖的位置，并标记为
$Y_0$，
$Z_0$，
并将此作为搜索中心，并记
...
$a_Y$，
$a_Z$
的范围都是[-1,1]，表示下一部动作朝Y、Z的正/负方向移动的权重。
$a_Y$,
$a_Z$
只是表示下一步的动作，而并非真实移动的距离，目前v1方案使用的是**位置增量**的方法，就是设定一个最大的动作步长
$\Delta{a_{yz}}=0.2mm$
，即
...
$Y_{hole}$、
$Z_{hole}$
为孔心。当满足上述条件以后，判定为找到孔心，此时SAC不再控制机器人，进入INSERTING阶段，交回底层控制器负责后续插入。
...
| `learning_starts` | 3000 | 学习前先随机收集多少步 | 随机探索阶段更长 |
...
| 参数 | 当前值 | 直接影响 |
|---|---:|---|
| SAC 动作周期 | 0.1 s | 每秒决定多少次 |
| SAC 频率 | 10 Hz | 同上 |
| 最大动作步长 | ±0.2 mm/轴 | 每次 reference 能跳多远 |
| 搜索范围 | ±4 mm/轴 | 最大搜索区域 |
| 最大搜索时间 | 40 s | 一回合允许搜索多久 |
| 最大搜索步数 | 400 | SAC 最多行动几次 |
| Y/Z 搜索刚度 | 80 N/m | 机械臂多积极追 SAC reference |
| Y/Z 搜索阻尼 | 70 N·s/m | 追踪时抑制振荡 |
| X 搜索推力 | 1 N | 轴压在板面的力度 |

**当前版本与INSERTING阶段相关的参数：**

| 参数 | 当前值 | 直接影响 |
|---|---:|---|
| Y/Z 插入刚度 | 900 N/m | 插入时强力保持孔中心 |
| Y/Z 插入阻尼 | 110 N·s/m | 抑制振荡 |
| X 插入刚度 | 600 N/m | 接近目标插入深度时的 X 向深度位置闭环 |
| X 插入阻尼 | 30 N·s/m | 抑制振荡 |
