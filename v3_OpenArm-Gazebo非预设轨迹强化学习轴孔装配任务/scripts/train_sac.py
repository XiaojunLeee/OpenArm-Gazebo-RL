...
class EpisodeStopper(BaseCallback):
    ...
    @staticmethod
    def _adjacent_action_angle(current_action, previous_action):
        """返回当前动作与上一动作的最小夹角，范围为 0～180 deg。

        如果没有上一动作，或任一动作模长接近 0，则返回 None。
        """
    ...
    def _on_step(self) -> bool:
        ...
        for env_index, (info, done) in enumerate(zip(infos, dones)):
            ...
            if self.logged_episode != current_episode:
                ...
                self.episode_action_step = 0
                self.previous_logged_action = None
            ...
            if env_index < len(actions) and actions.shape[1] >= 2:
                ...
                current_logged_action = np.asarray(
                    [action_y, action_z],
                    dtype=np.float64,
                )
                turn_angle_deg = self._adjacent_action_angle(
                    current_logged_action,
                    self.previous_logged_action,
                )
                turn_angle_text = (
                    'N/A'
                ...
                self.previous_logged_action = current_logged_action.copy()
